...
| `list_messages` | `lineId`, `limit` (1–200, default 20) | Recent texts and call transcripts, newest first: `from`, `body`, `code`, `receivedAt`, `label`, `service`. |
| `latest_code` | `lineId`, `from?`, `service?` | The most recent code already on the line, or `found: false`. |
...
Cleat already runs a hosted MCP server at `https://cleat.so/api/mcp`, listed in the MCP registry as `so.cleat/cleat`. It exposes the same four tool names with the same arguments, so a prompt written for one works with the other. Pick on these differences:
...
2. Verify your identity once, with a government ID. Until the workspace owner has verified, the API answers `403` and returns nothing: the line runs and keeps every text, but nobody can read them.
